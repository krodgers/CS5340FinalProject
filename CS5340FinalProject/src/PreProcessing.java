...
public class PreProcessing {
	...
	/**
	 * Strips the XML tags out of the file. Creates a list of NounPhrases that
	 * are in the coref brackets
	 * 
	 * @param file
	 *            the xml file
	 */
	public void stripXML(File file) {
		...
		try {
			...
			for (int x = 0; x < corefNodes.getLength(); x++) {
				// NounPhrase temp = new
				// NounPhrase(corefNodes.item(x).getTextContent());
				// corefs.add(temp);
			}
	...
	private ArrayList<String[]> tokenPosTags;// tags directly mapped to the
												// tokenizedSentence ArrayList
	// open nlp tools
	InputStream modelIn = null;// different models need to be loaded for each
								// task
	...
	public ArrayList<String> splitSentences(String currentChunk,
		...
		try {
			modelIn = new FileInputStream(currentDir
					+ "/classifiers/en-sent.bin");
			model = new SentenceModel(modelIn);
			splitter = new SentenceDetectorME(model);

			// open the temp out file and parse the paragraphs one line at a
			// time
			...
			line = line.trim();
			temp = splitter.sentDetect(line); // run the splitter which returns
												// an array
			for (int i = 0; i < temp.length; i++)
				// add each split sentence to the list of sentences
	...
	/**
	 * Tokenizes the sentence arrayList. The result of the tokenizer is an array
	 * of the sentence split up into tokens this method puts those arrays into
	 * an arrayList of token arrays. could possibly read out to file if that is
	 * desired.
	 */
	...
	/**
	 * NOT USED --combines the tokenized sentences with the tags in the form
	 * "word/tag".
	 * 
	...
	public ArrayList<String[]> fuseTagsForFunAndProfit() {
		...
		for (int i = 0; i < tokenizedSentences.size(); i++) {
			String[] tempToken = new String[tokenizedSentences.get(i).length];// this
																				// array
																				// will
																				// contain
																				// fused
			...
			for (int j = 0; j < tokenizedSentences.get(i).length; j++) {
				// fuse the tokens
				String fusedString = "";
				if (tokenPosTags.get(i)[j] != null
						&& tokenizedSentences.get(i)[j] != null) {
					fusedString = tokenizedSentences.get(i)[j];
					fusedString += "/" + tokenPosTags.get(i)[j];

				}
				tempToken[j] = fusedString;// add the fused string to the temp
											// tokenized sentenc
			}
			fused.add(tempToken);
		}
	 ...
	 * @return
	 */
	public String[] partialParse(ArrayList<String[]> sentArr,
			ArrayList<String[]> posArr) {
		ChunkerME chunker;
		ChunkerModel model;
	 ...
	 * @return
	 */
	private String[] chunksToNP(ArrayList<String[]> chunks,
			ArrayList<String[]> tokSent) {
		ArrayList<String> NPs = new ArrayList<String>(chunks.size() * 2);

		for (int i = 0; i < chunks.size(); i++) {// cycle through each sentence
			int arrSize = chunks.get(i).length;// cycle through the sentences np
												// chunks
	...
	public void FindNer(NounPhrase nounPhrase,
		...
		String classification = classifier.classifyWithInlineXML(nounPhrase
				.getPhrase());
		setNE(classification, nounPhrase);
	}

	/**
	 * This method takes a Named Entity classification(created by FindNER which
	 * uses Stanford NER) and extracts the noun phrases and puts them into the
	 * np sent in as a param.
	 * 
	 * @param np
	...
	private void setNE(String classification, NounPhrase np) {
		...
		try {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				np.addNamedEntity(nodes.item(i).getTextContent(),
						NounPhrase.Classification.ORGANIZATION);
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				np.addNamedEntity(nodes.item(i).getTextContent(),
						NounPhrase.Classification.PERSON);
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				String bob = nodes.item(0).getTextContent();
				np.addNamedEntity(nodes.item(0).getTextContent(),
						NounPhrase.Classification.MISC);
				// System.out.println(bob + " Loc");
			}
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				String bob = nodes.item(0).getTextContent();
				np.addNamedEntity(nodes.item(0).getTextContent(),
						NounPhrase.Classification.DATE);
				// System.out.println(bob + " Loc");
			}
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				String bob = nodes.item(0).getTextContent();
				np.addNamedEntity(nodes.item(0).getTextContent(),
						NounPhrase.Classification.MONEY);
				// System.out.println(bob + " Loc");
			}
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				String bob = nodes.item(0).getTextContent();
				np.addNamedEntity(nodes.item(0).getTextContent(),
						NounPhrase.Classification.PERCENT);
				// System.out.println(bob + " Loc");
			}
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				String bob = nodes.item(0).getTextContent();
				np.addNamedEntity(nodes.item(0).getTextContent(),
						NounPhrase.Classification.TIME);
				// System.out.println(bob + " Loc");
			}
	...
	/**
	 * Checks to see if the nounphrase contains pronouns. Sets the nounPhrase's
	 * pronoun boolean to true if it's phrase contains a pronoun. See
	 * parserUtil.tagIsPronoun for tags that are considered prounouns.
	 * 
	 * @param phrase
	 */
	...
	/**
	 * This will determine if a nounphrase is plural or not. See parserUtil for
	 * tags that are considered plural
	 * 
	 * @param phrase
	 *            the phrase to check
	 */
	...
	/**
	 * This method will create a nounphrase from a npTree. Then, this method
	 * will call all methods requisite to populating the new noun phrase's
	 * properties.
	 * 
	 * @param npTree
	 * @param classifier
	...
	public NounPhrase createNP(Tree npTree,
			AbstractSequenceClassifier classifier, Dictionaries d) {
		// extract pos tags
		...
		// find head nouns
		CollinsHeadFinder headFinder = new CollinsHeadFinder();// Stanford nlp's
																// head finder
		Tree head = headFinder.determineHead(npTree);// exctracts the head from
														// the npTree argument
		String headPhrase = "";// need to reconstruct the head before putting it
								// in the NP head variable
		...
		findGender(temp,d);
		temp.setArticle(parserUtil.CheckForArticle(temp.getPosTags(), temp.getPhraseAsList()));
		/*
		 * remove determiners find gender
		 * 
		 * all need to be done
		 */
		...
		for (Tree t : npTree) {
			if (t.value().equals("S")) {
				for (Tree s : t)
					...
					&& (t.value().equals("NP") || t.value().equals("PP")))
	 ...
	 * @param corefNP
	 */
	private void findGender(NounPhrase nounPhrase, Dictionaries d) {
		if(d.femaleWords.contains(nounPhrase.getHeadPhrase().trim().toLowerCase()))
				nounPhrase.setGender(NounPhrase.Gender.FEMALE);
		else if(d.maleWords.contains(nounPhrase.getHeadPhrase().trim().toLowerCase()))
			nounPhrase.setGender(NounPhrase.Gender.MALE);
		else if(d.femalePronouns.contains(nounPhrase.getHeadPhrase().trim().toLowerCase()))
			nounPhrase.setGender(NounPhrase.Gender.FEMALE);
		else if(d.malePronouns.contains(nounPhrase.getHeadPhrase().trim().toLowerCase()))
			nounPhrase.setGender(NounPhrase.Gender.MALE);
		else if(d.personPronouns.contains(nounPhrase.getHeadPhrase().trim().toLowerCase())||d.possessivePronouns.contains(nounPhrase.getHeadPhrase().trim().toLowerCase()))
